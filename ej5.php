...
function Sobrepeso($nombreperro , $datosperro){
    foreach($datosperro as $perro){
        if($perro["nombre"]=== $nombreperro){
            $edadhumana = edadhumana($perro["edad"]);
            if($edadhumana>=60 && $perro["peso"]>= 7){
                return true;
            }elseif($edadhumana<40 && $perro["peso"]>=3){
                return true;
            }if ($perro["peso"]>=6){
                return true;
            }else{
 ...
 function porcentaje($datosperro){
     $contador=0;
     foreach($datosperro as $perro){
        if(Sobrepeso($perro["nombre"]), $datosperro){
            $contador++;
       }
   }
   $porcentaje=$contador/count($datosperro) * 100;
   return $porcentaje
 }
$porcentaje= porcentaje($data)
echo "$porcentaje% de los perros tiene sobre peso";
